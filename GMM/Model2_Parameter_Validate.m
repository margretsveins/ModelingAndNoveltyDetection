...
for d = 1:1%length(Data)
    ...
    [Dist_test, Dist_train, num_outlier_test, num_K] = Model2_eMax(DataTrain, e_max_vec, iter);

    Dist_test_e_max = mean(Dist_test);
    Dist_train_e_max = mean(Dist_train);
    mean_num_outlier = mean(num_outlier_test);
    num_K_emax = mean(num_K);
    
    Dist_test_alphat{1,d} = Dist_test;
    Dist_train_alphat{1,d} = Dist_train;
    num_outlier_alphat{1,d} = num_outlier_test;
    num_K_emax_alphat{1,d} = num_K;
...
Dist_test_alphat_13to14 = {};
Dist_train_alphat_13to14 = {};
num_outlier_alphat_13to14 = {};
num_K_alphat_13to14 = {};
var_target = 77;
alpha_0 = 0.1:0.1:1;
tau_alpha = 0.2:0.2:2;
...
for d = 1:length(Data)
    ...
    Dist_test_alphat_13to14{1,d} = Dist_test;
    Dist_train_alphat_13to14{1,d} = Dist_train;
    num_outlier_alphat_13to14{1,d} = num_outlier_test;
    num_K_alphat_13to14{1,d} = num_K;
