...
noverlap = nwin * 0.5;
Data_noIED = {Data{1,2}, [Data{1,2} Data{1,3}], [Data{1,2} Data{1,3} Data{1,5}], [Data{1,2} Data{1,3} Data{1,5}, Data{1,6}]};
 j = 1;
Y_train_cell = {};
Y_test_cell = {};
for k = 1:4 %length(Data)
    % train data 
    %Data_train = Data_pre{1,k};
    Data_train = Data_noIED{1,k};
    num_window_train = floor(size(Data_train,2)/length_window);
    E_train = zeros(6*14,num_window_train);
    j = 1
    for i = 1:num_window_train
        % Get "window" of the data to summerize
        eeg_data = Data_train(:,j:j+length_window-1)';  
        % Get the features
        E = getFeatures_SBS2(eeg_data, nwin, sample_freq, length_window);
        E_train(:,i) = E;
        j = j+length_window;
    end
    ...
    E_test = zeros(6*14,num_window_test);
    j = 1
    ...
    k
    E_test_normal = (E_test - E_train_mean) ./ E_train_std;
...
%%
[Y_train_cell, Y_test_cell] = test_SBS2(Data);
...
for i =1:length(Y_train_cell)
    ...
    [log_p, log_p_train, log_p_test, ten_worst] = log_normal(Y_train, Y, 'Plot', true, title_plot{1,i}, plot_name{1,i}, frac_worst, 'Seizure', false, [SeizStart SeizEnd], 10);
%     figure()
